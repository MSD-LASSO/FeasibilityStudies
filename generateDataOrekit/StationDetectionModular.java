...
public class StationDetectionModular {
  ...
  // System.out.println(groundStations[1].getLatitude());
   ...
   /*
   for (int g=0; g<stationOverlap.size();g++) {
	   System.out.println(stationOverlap.get(g).getState().getDate());
   }
   */
	/* //BLOCK OF CODE TO CHECK SATELLITE LAT LONGITUDE /////////////////////////////////////////////////////////////
   ...
   while (extrapDate.compareTo(finalDate) <= 0)  {
    ...
    //   System.out.println(positionVectorSatellite.getX());
  ...
  ///*    PR
   ...
   int stationReference=1;
   String stationNo=Integer.toString(stationReference);
   //defining writer object to write to text file.
   PrintWriter unWriter= new PrintWriter("/Users/mtruong/Desktop/JAVA/testOrbitDataOneRoofV3.txt", "UTF-8");
   ...
   for (int b=0; b<groundStations.length;b++) {
	...
	unWriter.printf("%.15f\t%.15f\t%.15f\t%d \n",groundStations[b].getLatitude(),groundStations[b].getLongitude(),groundStations[b].getAltitude(),b);
//	unWriter.printf("%.3f %.3f %.3f \n",groundStations[b].getLatitude(),groundStations[b].getLongitude(),groundStations[b].getAltitude());
   ...
   PrintWriter positionWriter= new PrintWriter("/Users/mtruong/Desktop/JAVA/testOrbitDataAroundRIT_V3_POSITION.txt","UTF-8");
   ...
   while (ephemerisPropagateTime.compareTo(ephemerisEndTime)<=0) {
     ...
     //  System.out.println(positionVectorSatellite.getNorm());
      ...
      System.out.format("Elevation: %6.15f   Azimuth: %8.15f    Range: %12.15f Time: %s %n" ,elevation*180/Math.PI, azimuth*180/Math.PI,range,ephemerisPropagateTime.getDate().toString());
       ...
       //System.out.println(satLatLonAlt);
       double latitude=satLatLonAlt.getLatitude();
       double longitude=satLatLonAlt.getLongitude();
       double altitude=satLatLonAlt.getAltitude();
       System.out.format("Latitude: %3.15f	Longitude: %3.15f	Altitude: %3.15f %n", //%n
    		   latitude*180/Math.PI,longitude*180/Math.PI,altitude);//*180/Math.PI);
      ...
      // System.out.println(stationFrames[stationReference].pointAtDistance(azimuth, elevation, range));
       ...
       ephemerisPropagateTime = ephemerisPropagateTime.shiftedBy(60); //getting info every x seconds.
   }
   
    kepler.resetInitialState(stationOverlap.get(entryIndex+1).getState()); //getting last state
	...
	//debugging stuff for radiation pattern thinking
	//AbsoluteDate entryPoint=stationOverlap.get(entryIndex).getState().getDate();
	//double numberOfEntries= stationOverlap.get(entryIndex+1).getState().getDate().durationFrom(entryPoint)/15;
