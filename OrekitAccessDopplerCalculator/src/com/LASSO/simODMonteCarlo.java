...
//                         home.getAbsolutePath());
   ...
   AbsoluteDate initialDate = new AbsoluteDate(2019, 12,8 , 0, 0, 00.000, utc);
   
   
  AbsoluteDate epoch = new AbsoluteDate(2019, 12,8,0, 0, 00.000, utc);
   ...
   //Sun Synchronous Approximation
   double mu =  3.986004415e+14;
   double a = 7371000;                     // semi major axis in meters
   double e = 0.005;                   // eccentricity
   double i = 1.736602606;        // inclination
   double omega = 0.5;// // perigee argument
   double raan = 2.3;  // right ascension of ascending node
   double lM = 0.971205 ;                   // mean anomaly
   
   KeplerianOrbit keplerOrbit = new KeplerianOrbit(a, e, i, omega, raan, lM, PositionAngle.MEAN,
           inertialFrame, epoch, mu);
   
   //CircularOrbit keplerConvertedOrbit= new CircularOrbit(keplerOrbit);
  // keplerOrbit.
 ...
 ///* Ground Station Triangle
  ...
  ///*
   ...
   AbsoluteDate finalDate = new AbsoluteDate(initialDate, 100000, utc);  //end time, 60000 seconds after start.
   
  // double degreeError=0.5;
   ...
   System.out.format("# of passes: %d \n", stationOverlap.size());
   //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

   int monteIterations=50;
   double degreeError=2.5;  
   ArrayList<Double> eccentricityArray=new ArrayList<Double>(1);
   ArrayList<Double> semimajorArray=new ArrayList<Double>(1);
   ArrayList<Double> inclinationArray=new ArrayList<Double>(1);
   ArrayList<Double> meanAnomalyArray=new ArrayList<Double>(1);
   ArrayList<Double> rightAscensionArray=new ArrayList<Double>(1);
   ArrayList<Double> perigeeArgArray=new ArrayList<Double>(1);
	   
   
for (int p=0; p<monteIterations;p=p+1)   
{
	  ...
	  int entryIndex=b; //must be even. CHOOSING WHICH PASS TO LOOK AT
	   ...
	   while (ephemerisPropagateTime.compareTo(ephemerisEndTime)<=0) {
	    ...
	    //		   latitude,longitude,altitude);   //printing for debugging
	       ...
	       // adding az el and timestamps of flyby to arrays   %adding error
	       double rand_dub1=rand.nextGaussian()*degreeError*Math.PI/180;
	       double rand_dub2=rand.nextGaussian()*degreeError*Math.PI/180;
	     //  System.out.println(rand_dub1*180/Math.PI);
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	   	   double adjustedAzimuth=azimuth+rand_dub1;
	   	   double adjustedElevation=elevation+rand_dub2;
	       
	   	
	       azArray.add(adjustedAzimuth);
	       elArray.add(adjustedElevation);
	       timeStamps.add(ephemerisPropagateTime);
	       System.out.println(elevation*180/Math.PI);//*180/Math.PI);
		...
		double azEl_sigma =  0.02; //Estimated covariance of the range measurements, in meters
	    ...
	    System.out.format("successful degree error %.2f \n",degreeError);
	    ...
	    eccentricityArray.add(estimatedOrbit_init.getE());
	    semimajorArray.add(estimatedOrbit_init.getA());
	    inclinationArray.add(estimatedOrbit_init.getI());
	    meanAnomalyArray.add(temp.getMeanAnomaly());
	    rightAscensionArray.add(temp.getRightAscensionOfAscendingNode());
	    perigeeArgArray.add(temp.getPerigeeArgument());
	...
	PrintWriter unWriter= new PrintWriter("/Users/mtruong/Desktop/JAVA/MonteCarlo/monteTestData3GT.txt", "UTF-8");
	unWriter.printf("%d \n", monteIterations);
	// unWriter.printf("%s\t%s\t%s\t%s\n","Latitude","Longitude","Altitude","Station");
	
	unWriter.printf("e: %s \n", eccentricityArray.toString().replaceAll("[,\\[\\]]",""));
	unWriter.printf("a: %s \n", semimajorArray.toString().replaceAll("[,\\[\\]]",""));
	unWriter.printf("i: %s \n", inclinationArray.toString().replaceAll("[,\\[\\]]",""));
	unWriter.printf("mean an: %s \n", meanAnomalyArray.toString().replaceAll("[,\\[\\]]",""));
	unWriter.printf("righ asc: %s \n", rightAscensionArray.toString().replaceAll("[,\\[\\]]",""));
	unWriter.printf("peri arg: %s \n", perigeeArgArray.toString().replaceAll("[,\\[\\]]",""));
