...
orig_data = 'test_data_one.wav';
my_data = 'my_data.wav';
delayed_data = 'my_data_delayed.wav';
[~, Fs, N1] = readIQ(orig_data);
Time = N1/Fs;
make_IQ(1e3, Fs, Time, my_data);
plot_fft_IQ(my_data);
delay_IQ(my_data, input_delay, delayed_data);
[xm, ~, ~] = readIQ(my_data);
obw(xm, Fs)

% sum = 0;
% for i = 1:N
%     sum = sum + (Ac*abs(ym(i)))^2;
% end
% avg = sum / (N1/Fs);

add_noise_IQ(my_data, 20, my_data);
% add_noise_IQ(delayed_data, 10000, delayed_data);
[ym, ~, ~] = readIQ(my_data);
obw(ym, Fs);
% power1 = bandpower(ym);
% power2 = rms(ym)^2;
...
plot_fft_IQ(my_data);
[delay, ~, ~, ~, ~] = cmplx_xcorr_IQ(my_data, delayed_data, 0);
